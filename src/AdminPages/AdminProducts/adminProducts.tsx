  ...
  return (
    <main className="w-full h-screen flex flex-row">
      ...
      <section className="flex flex-col p-10 ml-20 w-full gap-5">
        <h1 className="text-4xl text-neutral-300">Product Dashboard, delete, create, edit!</h1>
        ...
        <div className="w-full lg:block">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 max-xl:gap-4 gap-6 mb-10">
            {products.map((product) => (
              <AdminProductCard key={product.id} productKey={product.id} name={product.name} price={product.price} />
            ))}
          </div>
        </div>
      </section>
