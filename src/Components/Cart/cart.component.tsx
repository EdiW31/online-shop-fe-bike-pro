...
const Cart = () => {
    ...
    const [products, setProducts ] = useState<Product[]>([]);  // Assuming this is the list of products in the cart
    ...
    const seeOrder = async () => {
        ...
        const productIds: any[] = orders.flatMap((order: any) => order.items.map((items: any) => items.productId));
        // setProducts(await Promise.all(productIds.map((id: any) => getProductById(id))));
        Promise.all(productIds.map(id => {
          ...
          .then(products => {
            console.log("Products after Promise.all:", products);
